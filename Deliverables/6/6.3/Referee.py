...
from CustomExceptions import ContractViolation, InvalidCommand
...
class Referee:
    ...
    def check_placements(self, placements):
        ...
        :rtype: list
        """
        if not isinstance(placements, list) or len(placements) != 2 or placements[0] == placements[1]:
        ...
        for placement in placements:
            ...
            if not RuleChecker.is_legal_placement(self.board.board, placement):
                raise ContractViolation("Invalid placement position given: {}".format(placement))
    ...
    def check_play(self, play):
        ...
        if not isinstance(play[1], list)\
            ...
            raise ContractViolation\
                ("Expected a non-empty list (max length = 2) of strings for directions. Received {}".format(play[1]))
        if self.player_turn is None:
            raise InvalidCommand("Both players must be registered before making other commands.")
        if play[0][:-1] != RuleChecker.COLORS[self.player_turn]\
                or not RuleChecker.is_legal_play(self.board, play[0], play[1]):
            return self.player_names[self.player_turn * -1 + 1]
        # TODO: add a check for is_winning_build
        # SEE ABOVE: argument modification required
